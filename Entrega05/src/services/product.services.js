import FileManager from './file.services.js';
...
export default class ProductManager extends FileManager{
    ...
    validarProducto = (title,
        ...
        category,
        thumbail)=>{
            if (title === null || title === "" ||descripcion ===null || descripcion ===""
                ||code === null || code === "" || price === null || price === "" ||stock === null ||stock === ""
                || status == "" || category === null || category === "" || thumbail ===null || thumbail ==="") {
                return false;
            }
        return true;
    } 
    ...
    addProduct = async(
        ...
        try{
            ...
            agregamos
            */
            if(products.some(element=>element.code === code)) throw 'No se pueden crear elementos con code repetido';
            if(this.validarProducto(title,descripcion,code,price,status,stock,category,thumbail)){
    ...
    deleteProduct = async (id) => {
        try{
            ...
            if(productDelet) return this.set(products.filter((element) => element.id != productDelet.id));
            throw 'No existe el producto,no se puede eliminar';
        }catch(e){throw e;}
    }
    updateProduct = async (id,title, descripcion, code, price, status, stock, category, thumbail) => {
        try{
            const products = await this.get(); //Leemos todos los productos
            if(!products.find(element=>element.id == id)) throw ('No existe el producto,no se puede modificar'); //verificamos que exista 
            const product = {id,title, descripcion, code, price, status, stock, category, thumbail};
